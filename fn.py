...
ERRORS = {
    # File-related errors
    '[F-0]': "The file size should not exceed 64GB.",
    '[F-1]': "You have selected one or more files larger than 64GB.",
    '[F-2]': "One or more selected files do not exist. Please check the filenames and try again.",
    
    # Connection and update-related errors
    '[F_UC-0]': "Update may be in progress. Please try again later or contact support if the issue persists.",
    '[F_UC-1]': "Failed to connect. Please verify your internet connection and try again.",
    '[F_UC-2]': "HTTP error occurred while connecting. Please try again.",
    '[F_UC-3]': "Unable to connect. Please check your network settings.",
    '[F_UC-4]': "Connection timed out. Please ensure you're connected to the internet and try again.",
    
    # Database-related errors
    '[DB_KNS-0]': "An issue was detected with the database file. Please ensure it is formatted correctly.",
    '[DB_KNS-1]': "Database file is missing. Please ensure the database file exists and is accessible.",
    '[DB_DBFC-0]': "An unexpected error occurred with the database file. Please check its integrity.",
    '[DB_DKNC-0]': "Database file appears to be corrupt or unreadable. Please verify the file's integrity.",
    
    # AES encryption-related errors
    '[AES_E-0]': "This encryption key has already been used. Please use a unique key.",
    '[AES_E-1]': "Encryption key must be up to 32 characters long. Please adjust your input.",
    '[AES_E-2]': "Encryption key must be up to 32 characters long. Please adjust your input.",
    '[AES_E-3]': "The specified file does not exist. Please check the file path and try again.",
    
    # AES decryption-related errors
    '[AES_D-0]': "Both key and nonce are required for decryption. Please enter both values.",
    '[AES_D-1]': "Key format error: Please ensure your key is in the correct hex format.",
    '[AES_D-2]': "Nonce format error: Please ensure your nonce is in the correct hex format.",
    '[AES_D-3]': "Please verify that your key and nonce are correctly formatted and try again.",
    '[AES_D-4]': "Decryption key must be up to 32 characters long. Please adjust your input.",
    '[AES_D-5]': "The entered key or nonce is incorrect. Please double-check your input.",
    '[AES_D-6]': "This file is not encrypted or the encryption type is unrecognized.",
    
    # UI-related errors
    '[UI_DBB-0]': "An error occurred with the database file in the UI. Please verify the file's content.",
    '[UI_DBB-1]': "The UI encountered unreadable characters in a file. Please check the file for errors."
}


def format_message(mtype, message):
    """Helper function to format messages based on their type."""
    prefix = {
        'info': '[Info] - ',
        'warn': '[Warning] - ',
        'imp': '[Important] - ',
        'error': '[Error] - '
    }.get(mtype, '[Unknown Type] - ')

    # Append additional information for error messages
    if mtype == 'error':
        message += ' ' + ERRORS.get(message, "An unknown error occurred.")
...
def Logger(mtype, message):
    if mtype == 'fileslistimport':
        ...
        for item in message:
            UIWindow.Logger.appendPlainText(item)
    else:
        formatted_message = format_message(mtype, message)
        UIWindow.Logger.appendPlainText(formatted_message)


class AES_SYSTEM():
	def FnSystem(self):
		self.cur_enc_system = UIWindow.Fn_system_label.text()
		if UIWindow.Fn_system_label.text() == 'AES-EAX':
			return AES.MODE_EAX
		else:
			self.cur_fn_system = 'AES-GCM'
			return AES.MODE_GCM
	...
	def Encrypt(self):
		for self.address in UIWindow.files_list:
			...
			## ENCRYPTION KEY
			if UIWindow.fn_key_input.text(): # KEY
				if UIWindow.enc_key_label.text() == 'Key (B):': # Bytes format key
					## Check key's length
					if len(UIWindow.fn_key_input.text()) <= 31:
						self.key = pad(UIWindow.fn_key_input.text().encode() ,32) # pad key in total of 32 bytes (256bit)
					elif len(UIWindow.fn_key_input.text()) == 32:
						self.key = UIWindow.fn_key_input.text().encode()
					else:
					...
					try:
						self.key = UIWindow.fn_key_input.text()
			...
			## NONCE
			cipher = AES.new(self.key, self.FnSystem()) # AES Encryption System
	...
	def Decrypt(self):
		for self.address in UIWindow.files_list:
			...
			try:
				cipher = AES.new( self.key, self.FnSystem(), nonce=self.nonce ) # AES Encryption System
			except ValueError:
...
class DB():
		...
		if self.DBFileChecker():
			with open(UIWindow.DATABASE_FILE, 'r+') as DB_file:
				try:
					...
					data[self.filehash] =  [self.key.hex() , self.nonce.hex(), self.tag.hex(), UIWindow.Fn_system_label.text(), self.address]
					DB_file.seek(0)
	...
	def KeyNonceSearcher(self): # { hash : [ key, nonce, tag, fn_system ] }
		if self.DBFileChecker():
			try:
				with open(UIWindow.DATABASE_FILE, 'r') as DB_file:
					data = json.load(DB_file)
					...
					if self.filehash in data:
						...
						UIWindow.Fn_system_label.setText(data[self.filehash][3])
						return True
					else:
						return False
		...
		if all([path.exists(x) for x in address_list]): # Check if all files exist
			...
			if len(address_list) == 1: # If only one file has been selected
				...
				if self.filesize < 64: # If file is >= 64GB, you should split file to smaller parts cause of encryption security reasons
					...
					UIWindow.fn_key_input.setEnabled(True)
				...
				if all([ os.path.getsize(x)/pow(1024,3) < 64 for x in self.addresses]): # Check if all files are <64GB
					...
					UIWindow.fn_key_input.setText('')
					UIWindow.fn_key_input.setEnabled(False)
					UIWindow.dec_key_input.setText('')
					UIWindow.dec_nonce_input.setText('')
...
class UI(QMainWindow):
	...
	def __init__(self):
		...
		uic.loadUi("FN_gui.ui",self) # Load Main GUI
		self.setWindowTitle('FN-crypt') 
		...
		clickable(self.Fn_system_label).connect(lambda label_name="fn_system_label": self.LabelSwitcher(label_name) )
	...
	def LabelSwitcher(self, label_name):
		if label_name == 'fn_key_label':
			if self.enc_key_label.text() == 'Key (B):':
				self.enc_key_label.setText('Key (H):')
				self.fn_key_input.setPlaceholderText('Type your key in Hex format (example: 1341534511436)')
			else:
				self.enc_key_label.setText('Key (B):')
				self.fn_key_input.setPlaceholderText('Type your key in Bytes format (example: rainbow)')

		elif label_name == 'dec_key_label':
			if self.dec_key_label.text() == 'Key (B):':
				self.dec_key_label.setText('Key (H):')
				self.dec_key_input.setPlaceholderText('Type your key in Hex format (example: 1341534511436)')
			else:
				self.dec_key_label.setText('Key (B):')
				self.dec_key_input.setPlaceholderText('Type your key in Bytes format (example: rainbow)')

		elif label_name == 'fn_system_label':
			if self.Fn_system_label.text() == 'AES-EAX':
				self.Fn_system_label.setText('AES-GCM')  # This are System label 
			else:
				self.Fn_system_label.setText('AES-EAX')
	...
	def SetDefaultOptions(self):
		...
		self.Fn_system_label.setText(options['Fn_system_label'])
		OPT_file.close()
	...
	def SaveOptions(self):
		...
		with open('Options.txt', 'w') as OPT_file:
			options = {
					...
					"Fn_system_label":self.Fn_system_label.text()
					}
			json.dump(options, OPT_file)
